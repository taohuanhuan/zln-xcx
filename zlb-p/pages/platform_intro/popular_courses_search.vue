<template>
	<view class="container" style="width:690rpx">
		...
		<view class="" style="background-color: #ffffff;margin: 0 -30rpx;">
			<u-tabs :scrollable="true" :list="tabs" @click="changeTabs" lineWidth="30" lineHeight="5" lineColor="#C871E7"
				:activeStyle="{
			        fontWeight: 'bold',
			    }"></u-tabs>
		</view>
		...
		<!-- 课程列表 -->
		<view v-for="(course, index) in courses" :key="index" class="course-list-item" @click="navigateToCourseDetailso(course.course_id,course.course_type)">
...
<script>
import { getHotCourseList,getDataDictionaryList } from '@/config/api'; // 从统一文件中引入API
...
export default {
	...
	methods: {
		...
		async fetchHotCourses(id) {
			...
			console.log('热门课程:', response); // 打印返回的数据
		},
	...
	async onLoad() {
		// 页面加载时调用获取热门课程的请求
		let DictionaryList = await getDataDictionaryList({data_code:'course_category_code'})
		
		DictionaryList.forEach((item)=>{
			this.tabs.push({id:item.data_key,name:item.data_value})
		})
		
		this.fetchHotCourses(this.tabs[0].id);
	}
};
</script>
